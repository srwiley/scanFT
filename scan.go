// 2017: This is the anti-aliasing algorithym from the golang
...
func (s *ScannerFT) setCell(xi, yi int) {
	if s.xi != xi || s.yi != yi {
		s.saveCell()
		s.xi, s.yi = xi, yi
	}
...
func (s *ScannerFT) scan(yi int, x0, y0f, x1, y1f fixed.Int26_6) {
...
func (s *ScannerFT) areaToAlpha(area int) uint32 {
...
// to the painter are non-overlapping, and sorted by Y and then X. They all have non-zero
